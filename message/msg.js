...
module.exports = async(conn, msg, m, setting, store, welcome) => {
	try {
		...
		const textImg = (teks) => {
			return conn.sendMessage(from, { text: teks, jpegThumbnail: pp_bot }, { quoted: fdoc })
		}
...
limitAdd(sender, limit)
			...
			case prefix+'getvideo': case prefix+'getvidio':
			    ...
			    y2mateV(`https://youtube.com/watch?v=${arrey[args[1] -1]}`).then( data => {
			      ...
			      conn.sendMessage(from, { video: { url: data[0].link }, caption: capt }, { quoted: fdoc })
			       limitAdd(sender, limit)
				}).catch(() => reply(mess.error.api))
		        break
            ...
            case prefix+'mediafire':
			    ...
			    reply(mess.wait)
					...
					conn.sendMessage(from, { document: { url: data.url }, fileName: `${data.filename}`, mimetype: 'zip' }, { quoted: fdoc })
					limitAdd(sender, limit)
					break
			// Owner Menu
			case prefix+'exif':
  ...
  case prefix+'stalkgh':
    ...
    conn.sendMessage(from, {caption: caption, image: {url: data.result.avatar}}, {quoted: fdoc})
    limitAdd(sender, limit)
    break
			// Random Menu
			...
			case prefix+'cecan': case prefix+'cewek':
				...
				conn.sendMessage(from, { caption: "Random Cewe Cantik", image: { url: pickRandom(data.result) }, buttons: but, footer: 'Pencet tombol dibawah untuk foto selanjutnya' }, { quoted: fdoc })
			    limitAdd(sender, limit)
 			    break
			case prefix+'cogan': case prefix+'cowok':
				...
				var data = await pinterest(pickRandom(query))
				var but = [{buttonId: `${command}`, buttonText: { displayText: "Next Photo" }, type: 1 }]
				conn.sendMessage(from, { caption: "Random Cowo Ganteng", image: { url: pickRandom(data.result) }, buttons: but, footer: 'Pencet tombol dibawah untuk foto selanjutnya' }, { quoted: fdoc })
			    limitAdd(sender, limit)
				break
            case prefix+'artimimpi':
  if (args.length < 2) return reply(`Kirim perintah ${command} ular`)
			    if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
			    const artimimpi = args.join(" ")
				var data = await fetchJson(`https://hitomi-rest-api.herokuapp.com/api/artimimpi?text=${args[1]}&apikey=${setting.hitokey}`)
			    reply(`Mimpi : ${q}\n\nArti : ${data.result.result.list}`)
				limitAdd(sender, limit)
				break
  ...
  if (args.length < 2) return reply(`Kirim perintah ${command} ${setting.ownerName}`)
				...
				var data = await fetchJson(`https://hitomi-rest-api.herokuapp.com/api/artinama?text=${args[1]}&apikey=${setting.hitokey}`)
			    reply(`${data.result.result.list}`)
				limitAdd(sender, limit)
				break
            case prefix+'resepmasakan':
  if (args.length < 2) return reply(`Kirim perintah ${command} Bakso`)
			    if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
			    const resepmasakan = args.join(" ")
				var data = await fetchJson(`https://hitomi-rest-api.herokuapp.com/api/resepmasakan?text=${args[1]}&apikey=${setting.hitokey}`)
			    reply(`${data.result.list.nama} \n\n ${data.result.list.cara}`)
				limitAdd(sender, limit)
				break
            case prefix+'katajago':
  if (args.length < 2) return reply(`Kirim perintah ${command} Cantik`)
			    if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
			    const katajago = args.join(" ")
				var data = await fetchJson(`https://hitomi-rest-api.herokuapp.com/api/katajago?text=${args[1]}&apikey=${setting.hitokey}`)
			    reply(`Kata Jago : ${q}\n\nArti : ${data.result.result.list}`)
				limitAdd(sender, limit)
				break
...
var but = [{buttonId: `${command}`, buttonText: { displayText: "Next Photo" }, type: 1 }]
				conn.sendMessage(from, { caption: "DarkJokes", image: { url: hasil.result }, buttons: but, footer: 'Pencet tombol dibawah untuk foto selanjutnya' }, { quoted: fdoc })
limitAdd(sender, limit)
break
case prefix+'readmore':
  case prefix+'more':
    ...
    conn.sendMessage(from, { text: retmor}, { quoted: fdoc })
    break
			// Search Menu
  ...
  if (!isOwner)return reply("_Maaf Fitur Ini Di Nonaktifkan Oleh Owner, Karena menyebabkan nomer bot 3 kali ke banned_")
    ...
    break
			...
			case prefix+'ceklimit': case prefix+'cekbalance':
				...
				break
// Wallpaper Menu
case prefix+'wallml': case prefix+'wallpaperml':
			    if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
				reply(mess.wait)
				var data = await fetchJson(`https://hitomi-rest-api.herokuapp.com/api/wallml?apikey=${setting.hitokey}`)
				var but = [{buttonId: `${command}`, buttonText: { displayText: "Next Photo" }, type: 1 }]
				conn.sendMessage(from, { caption: "Random Wallpaper", image: { url: data.result.list.gambar }, buttons: but, footer: 'Pencet tombol dibawah untuk foto selanjutnya' }, { quoted: fdoc })
			    limitAdd(sender, limit)
				break
case prefix+'wallrandom': case prefix+'wallpaperrandom':
			    if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
				reply(mess.wait)
				var data = await fetchJson(`https://hitomi-rest-api.herokuapp.com/api/wallrandom?apikey=${setting.hitokey}`)
				var but = [{buttonId: `${command}`, buttonText: { displayText: "Next Photo" }, type: 1 }]
				conn.sendMessage(from, { caption: "Random Wallpaper", image: { url: data.result.list.gambar }, buttons: but, footer: 'Pencet tombol dibawah untuk foto selanjutnya' }, { quoted: fdoc })
			    limitAdd(sender, limit)
				break
case prefix+'wallcode': case prefix+'wallpapercode':
			    if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
				reply(mess.wait)
				var data = await fetchJson(`https://hitomi-rest-api.herokuapp.com/api/wallcode?apikey=${setting.hitokey}`)
				var but = [{buttonId: `${command}`, buttonText: { displayText: "Next Photo" }, type: 1 }]
				conn.sendMessage(from, { caption: "Random Wallpaper", image: { url: data.result.list.gambar }, buttons: but, footer: 'Pencet tombol dibawah untuk foto selanjutnya' }, { quoted: fdoc })
			    limitAdd(sender, limit)
				break
case prefix+'wallneon': case prefix+'wallpaperneon':
			    if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
				reply(mess.wait)
				var data = await fetchJson(`https://hitomi-rest-api.herokuapp.com/api/wallneon?apikey=${setting.hitokey}`)
				var but = [{buttonId: `${command}`, buttonText: { displayText: "Next Photo" }, type: 1 }]
				conn.sendMessage(from, { caption: "Random Wallpaper", image: { url: data.result.list.gambar }, buttons: but, footer: 'Pencet tombol dibawah untuk foto selanjutnya' }, { quoted: fdoc })
			    limitAdd(sender, limit)
				break
case prefix+'wallpubg': case prefix+'wallpaperpubg':
			    if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
				reply(mess.wait)
				var data = await fetchJson(`https://hitomi-rest-api.herokuapp.com/api/wallpubg?apikey=${setting.hitokey}`)
				var but = [{buttonId: `${command}`, buttonText: { displayText: "Next Photo" }, type: 1 }]
				conn.sendMessage(from, { caption: "Random Wallpaper", image: { url: data.result.list.gambar }, buttons: but, footer: 'Pencet tombol dibawah untuk foto selanjutnya' }, { quoted: fdoc })
			    limitAdd(sender, limit)
				break
